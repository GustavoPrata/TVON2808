...
async function getApiBase() {
  ...
  // Usa a lista de backends definida globalmente
  for (const server of BACKEND_URLS) {
    try {
  ...
  // Se nenhum servidor responder, tenta o primeiro da lista
  await logger.warn('⚠️ Nenhum servidor respondeu, usando primeiro da lista como fallback');
...
// Função para reconexão automática com retry exponencial
async function reconnectServer() {
  await logger.info('🔄 Iniciando processo de reconexão...');
  
  let delay = RECONNECT_CONFIG.retryDelay;
  ...
  for (let attempt = 0; attempt < RECONNECT_CONFIG.maxRetries; attempt++) {
    try {
      ...
      if (response.ok) {
        await logger.info('✅ Reconexão bem-sucedida!');
        API_STATUS.reconnectAttempts = 0;
        await ensureOfficeTabOpen(true);
        return true;
      }
    } catch (e) {
      await logger.warn(`❌ Tentativa ${attempt + 1} falhou: ${e.message}`);
    }
  ...
  await logger.error('❌ Todas as tentativas de reconexão falharam');
  return false;
}

// Sistema de auto-recuperação (não força abrir abas)
async function startAutoRecovery() {
  setInterval(async () => {
    // Apenas verifica se há abas abertas, não força abertura
    ...
    for (const tab of tabs) {
      if (tab.url && (
        ...
        hasOfficeTab = true;
        // Só recarrega se a aba já existe
        // chrome.tabs.reload(tab.id);
        await logger.info('📌 Aba do OnlineOffice detectada');
      }
    }
    
    if (!hasOfficeTab) {
      await logger.debug('📋 Funcionando em background sem aba aberta');
      // NÃO força abrir aba - funciona em background
    }
...
async function setupAlarms() {
  ...
  chrome.alarms.clear('pollBackend', async () => {
    // Cria novo alarme que dispara a cada 15 segundos (mais frequente para garantir processamento)
    chrome.alarms.create('pollBackend', {
      periodInMinutes: 0.25, // 15 segundos
      delayInMinutes: 0 // Começa imediatamente
    });
    await logger.info('⏰ Alarme configurado para polling automático a cada 15s');
  });
  
  // Cria alarme para manter service worker ativo
  chrome.alarms.create('keepAlive', {
    periodInMinutes: 0.5, // A cada 30 segundos
    delayInMinutes: 0
  });
  
  // Alarme para heartbeat com o backend
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  if (alarm.name === 'pollBackend') {
    ...
    await logger.debug('💓 Keep-alive ping');
...
// Função para enviar heartbeat ao backend
async function sendHeartbeat() {
  try {
    ...
    // Verifica se há abas abertas do OnlineOffice
    const tabs = await chrome.tabs.query({
    ...
    const response = await fetch(`${API_BASE}/api/extension/heartbeat`, {
      ...
      body: JSON.stringify({
        currentUrl: tabs[0]?.url || 'background',
    ...
    await logger.warn('⚠️ Erro ao enviar heartbeat:', { error: error.message });
  }
}
...
async function ensureOfficeTabOpen(forceOpen = false) {
  try {
    // NÃO força abertura de abas - apenas monitora se existem
    const tabs = await chrome.tabs.query({
      url: [
        '*://onlineoffice.zip/*', 
        '*://*.onlineoffice.zip/*',
    ...
    if (tabs.length === 0) {
      await logger.info('🔧 Funcionando em modo background - sem aba aberta');
      // NÃO abre aba automaticamente
      // A extensão funciona independentemente
      return null;
    }
    
    // Se já tem aba aberta, apenas retorna a primeira
    const existingTab = tabs[0];
    await logger.debug(`📌 Usando aba existente: ${existingTab.id}`);
    return existingTab;
    
  } catch (error) {
    await logger.error('❌ Erro em ensureOfficeTabOpen:', { error: error.message });
    return null;
  }
}
...
// Função para verificar e alternar backends automaticamente
async function ensureBackendConnection() {
  const now = Date.now();
  
  // Evita checagens muito frequentes
  if (now - API_STATUS.lastCheck < RECONNECT_CONFIG.retryDelay) {
    return API_STATUS.isHealthy;
  }
  
  API_STATUS.lastCheck = now;
  
  // Tenta cada backend na lista com retry exponencial
  for (let attempt = 0; attempt < RECONNECT_CONFIG.maxRetries; attempt++) {
    for (let i = 0; i < BACKEND_URLS.length; i++) {
      const backendUrl = BACKEND_URLS[i];
      try {
        ...
        if (response.ok) {
          if (backendUrl !== API_BASE) {
            await logger.info(`🔄 Mudando para backend: ${backendUrl}`);
            API_BASE = backendUrl;
            await chrome.storage.local.set({ 
              apiBase: backendUrl,
        ...
        await logger.warn(`❌ Backend ${backendUrl} não disponível: ${e.message}`);
        
        // Se for erro de timeout ou rede, tenta próximo mais rápido
    ...
    // Espera exponencial entre tentativas
    const delay = RECONNECT_CONFIG.retryDelay * Math.pow(RECONNECT_CONFIG.backoffMultiplier, attempt);
    await new Promise(resolve => setTimeout(resolve, delay));
  }
  
  API_STATUS.isHealthy = false;
  API_STATUS.reconnectAttempts++;
  
  if (API_STATUS.reconnectAttempts >= 5) {
    await logger.error('❌ Todos os backends falharam após 5 tentativas');
    // Reinicia a extensão após muitas falhas
