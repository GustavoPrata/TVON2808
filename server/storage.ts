...
export interface IStorage {
  ...
  registrarRenovacaoAutomatica(systemId: number, novaCredencial: {username: string; password: string}): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async createSistemaAutoGenerated(data: { 
    ...
    url?: string; 
    expiracao?: Date;
    systemId?: number; // ID do sistema para atualizar (renovação) 
  }): Promise<Sistema> {
    // Se systemId fornecido, está renovando um sistema existente
    if (data.systemId) {
      console.log(`🔄 Atualizando sistema ${data.systemId} com novas credenciais`);
      
      // Buscar o sistema existente
      const existingSistema = await this.getSistemaById(data.systemId);
      if (!existingSistema) {
        throw new Error(`Sistema ${data.systemId} não encontrado`);
      }
      ...
      const result = await db.update(sistemas)
        ...
        .where(eq(sistemas.id, data.systemId))
        .returning();
  ...
  async marcarRenovacaoFalhou(id: number, erro: string): Promise<void> {
    ...
    await db.insert(logs).values({
      ...
      detalhes: { systemId: id, erro },
      timestamp: new Date()
    });
  }

  async registrarRenovacaoAutomatica(systemId: number, novaCredencial: {username: string; password: string}): Promise<void> {
    await db.transaction(async (tx) => {
      ...
      await tx.insert(logs).values({
        nivel: 'info',
        origem: 'sistema_renewal',
        mensagem: `Sistema ${systemId} renovado automaticamente`,
        detalhes: {
          systemId,
  ...
  async updateSistemaActivePontos(systemId: number): Promise<void> {
    const [result] = await db.select({ count: count() })
      .from(pontos)
      .where(and(
        eq(pontos.systemId, systemId),
  ...
  async getOfficeCredentials(limit: number = 10000): Promise<OfficeCredentials[]> {
    ...
    for (const row of result) {
      ...
      if (systemId && !seenSystems.has(systemId)) {
        seenSystems.add(systemId);
        // Usar systemId ao invés do systemId interno para exibição
        uniqueCredentials.push({
          ...row,
          systemId: row.systemId ? parseInt(row.systemId) : systemId
        });
      }
    }
